...
Every dataset used in this repository is publicly available. This document lists each source, its origin, the license or terms of use, and how to re-download it. Files that exceed ~5 MB are not committed to git and must be downloaded locally before running the analysis scripts.
...
### USGS — soil lead
- **Files:** `usgs_soil_lead_by_state.csv`, `usgs_soil_lead_by_county_sample.csv`, `usgs_soil_lead_concentrations.csv` (large, gitignored)
- **Source:** [USGS National Geochemical Database — soils](https://mrdata.usgs.gov/soilgeochemistry/)
- **License:** U.S. government public domain

### EPA — predicted blood lead, LCR, Superfund, TRI
- **Files:**
  - `epa_predicted_county_blood_lead.csv` — EPA IEUBK-derived childhood BLL estimates
...
- **Files:** `real_census_acs_data.csv`, `census_demographics_for_lead.csv`, `census_race_ethnicity_by_county.csv`, `census_median_age_by_county.csv`, `census_living_alone_by_county.csv`, `census_pre1940_housing_by_county.csv`, `census_pre1978_housing_by_county.csv`, `census_pre1980_housing_by_county.csv`
...
- **Build:** `build_county_lead_master.py` merges the above sources by county FIPS. Re-running `build_county_lead_master.py` with the raw source files in place reproduces the master dataset.
...
- All U.S. federal datasets are public-domain and may be reused without restriction.
...
- Analytical code, figures, and manuscript text in this repository are released under the MIT License (see `LICENSE`).
